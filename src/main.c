    ...
    while (true) {
        ...
        while (cril_read_input(id, &input)) {
            if (input.type == CRIL_HOLD) {
                if (!menu) {
                    ...
                    if (input.key == BTN_TL) menu = false;
                    if (input.key == BTN_A) {
            ...
            if (input.type == CRIL_ANALOG) {
                if (!menu) {
                    if (input.axis == ABS_HAT0X && input.value > 0 && selected_y > 0) selected_x = (selected_x + 1) % 6;
                    if (input.axis == ABS_HAT0X && input.value < 0 && selected_y > 0) selected_x = selected_x == 0 ? 5 : selected_x - 1;
                    ...
                    if (input.axis == ABS_HAT0Y && input.value > 0) selected_y = (selected_y + 1) % 5;
                    if (input.axis == ABS_HAT0Y && input.value < 0) selected_y = selected_y == 0 ? 4 : selected_y - 1;
                } else {
                    if (input.axis == ABS_HAT0Y && input.value > 0) menu_selected = (menu_selected + 1) % 5;
                    if (input.axis == ABS_HAT0Y && input.value < 0) menu_selected = menu_selected > 0 ? menu_selected - 1 : 4;
                }
            }
        }
        ...
        if (error) {
            ...
            for (size_t i = 0; i < expression.size;) {
                while (i < expression.size && (isdigit(expression.data[i]) || expression.data[i] == '.')) {
                    number_size++; i++;
                }
                if (number_size) {
                ...
                if ((i >= expression.size || expression.data[i] != ')') && expression.data[i - 1] != '(') x += 8;
                number_size = 0;
            }
        ...
        for (int x = 0; x < 6; x++) {
            for (int y = 1; y < 5; y++) {
                ...
                csgl_rect_rounded(&state,
                width * x / 6 + 8, height * y / 5 + 8,
                width / 6 - 16, height / 5 - 16, 16, color);
        ...
        if (menu) {
            ...
            csgl_rect(&state, 0, 0, width, height, CSGL_RGBA_HEX(0x00000088));
            ...
            if (menu_selected == 1) csgl_rect(&state, 0, y, menu_width, 32, theme->bgp1);
            csgl_rect_rounded(&state, 8, y + 8, 16, 16, 8, theme->fg);
            if (theme != &gruber_darker) csgl_rect_rounded(&state, 10, y + 10, 12, 12, 6, menu_selected == 1 ? theme->bgp1 : theme->bg);
            csgl_text(&state, 32, y, font_24, "Gruber Darker", .color = theme->fg);
            y += 32;
            
            if (menu_selected == 2) csgl_rect(&state, 0, y, menu_width, 32, theme->bgp1);
            csgl_rect_rounded(&state, 8, y + 8, 16, 16, 8, theme->fg);
            if (theme != &nord) csgl_rect_rounded(&state, 10, y + 10, 12, 12, 6, menu_selected == 2 ? theme->bgp1 : theme->bg);
            csgl_text(&state, 32, y, font_24, "Nord", .color = theme->fg);
            y += 32;
            
            if (menu_selected == 3) csgl_rect(&state, 0, y, menu_width, 32, theme->bgp1);
            csgl_rect_rounded(&state, 8, y + 8, 16, 16, 8, theme->fg);
            if (theme != &gruvbox) csgl_rect_rounded(&state, 10, y + 10, 12, 12, 6, menu_selected == 3 ? theme->bgp1 : theme->bg);
            csgl_text(&state, 32, y, font_24, "Gruvbox", .color = theme->fg);
            y += 32 + 8;
            
            if (menu_selected == 4) csgl_rect(&state, 0, y, menu_width, 32, theme->bgp1);
